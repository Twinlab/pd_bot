...
- Создания розыгрышей с указанной длительностью и описанием
- Автоматического выбора победителя по истечении времени
- Отправки уведомлений о результатах розыгрыша
- Отслеживания активных розыгрышей и их корректного завершения

Пользователи могут участвовать в розыгрышах, нажимая на реакцию 🎉 под сообщением.
"""
...
from typing import Dict, List, Optional  # Убираем Set, Union; добавляем List
...
class GiveawayCog(commands.Cog):
    ...
    async def giveaway(self, ctx: commands.Context, duration: str, *, description: str) -> None:
        ...
        if duration_seconds is None:
            await ctx.send(
                (
                    "Неверный формат времени. Используйте 's' для секунд, 'm' для минут и "
                    "'h' для часов. "
                    "Например: 1h30m"
                )
            )
        ...
        embed = discord.Embed(
            title="🎉 Розыгрыш", description=description, color=discord.Color.green()
        )
        ...
        embed.add_field(
            name="Длительность", value=self.format_duration(duration_seconds), inline=True
        )
        ...
        logger.info(
            (
                f"Создан розыгрыш (ID: {giveaway_message.id}), длительность: "
                f"{self.format_duration(duration_seconds)}"
            )
        )

    async def wait_and_collect_reactions(
        self,
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Сообщение розыгрыша {giveaway_message.id} не найдено, возможно удалено"
                )
                # Удаляем из активных
            ...
            embed = discord.Embed(
                ...
                description=(
                    message.embeds[0].description if message.embeds else "Описание недоступно"
                ),
            ...
            if participants:
                ...
                embed.add_field(
                    name="Победитель", value=f"{winner.mention} ({winner.name})", inline=False
                )
            else:
                embed.add_field(
                    name="Победитель", value="Никто не участвовал в розыгрыше 😢", inline=False
                )
            ...
            if participants:
                ...
                await ctx.channel.send(
                    "🎉 Розыгрыш завершен, но никто не принял участие."
                    + f"\nСсылка на розыгрыш: {message.jump_url}"
                )
            ...
            if participants:
                participants_list = "\n".join(
                    [f"{i+1}. {user.name} ({user.id})" for i, user in enumerate(participants)]
                )
                ...
                await ctx.author.send(
                    f"**Список участников розыгрыша (всего {participants_count}):**"
                )
                for chunk in chunks:
            ...
            logger.info(
                (
                    f"Розыгрыш {giveaway_message.id} завершен. Победитель: "
                    f"{winner.name if participants else 'Нет'}. Участников: {participants_count}"
                )
            )
            ...
            logger.error(
                f"Ошибка в wait_and_collect_reactions для розыгрыша {giveaway_message.id}: {e}",
                exc_info=True,
            )
            try:
