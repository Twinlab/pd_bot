...
class TestTwitchDataManager:
    ...
    async def test_add_streamer_new(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.update_or_create", new_callable=AsyncMock) as mock_update_or_create:
    ...
    async def test_add_streamer_exception(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.update_or_create", side_effect=Exception("Test DB Error")):
            result = await twitch_manager.add_streamer(
                guild_id=1, channel_id=2, twitch_username="testuser", twitch_id="123456"
            )
    ...
    async def test_remove_streamer_exception(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.filter", side_effect=Exception("Test DB Error")):
            result = await twitch_manager.remove_streamer(guild_id=1, twitch_username="testuser")
    ...
    async def test_get_streamers_success(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.filter") as mock_filter:
            mock_streamer1 = MagicMock(
                guild_id=1, channel_id=2, twitch_username="user1", twitch_id="123",
                is_live=False, last_stream_id=None, last_notification_time=0
            )
            mock_streamer2 = MagicMock(
                guild_id=1, channel_id=3, twitch_username="user2", twitch_id="456",
                is_live=True, last_stream_id="s1", last_notification_time=100
            )
            
            # Имитируем awaitable результат filter()
            mock_filter_result = AsyncMock()
            mock_filter_result.__iter__ = MagicMock(return_value=iter([mock_streamer1, mock_streamer2]))
            # Для await filter()
    ...
    async def test_get_all_streamers_success(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.all", new_callable=AsyncMock) as mock_all:
            mock_streamer1 = MagicMock(
                guild_id=1, channel_id=2, twitch_username="user1", twitch_id="123",
                is_live=False, last_stream_id=None, last_notification_time=0
            )
            mock_streamer2 = MagicMock(
                guild_id=2, channel_id=3, twitch_username="user2", twitch_id="456",
                is_live=True, last_stream_id="s1", last_notification_time=100
            )
    ...
    async def test_get_all_streamers_exception(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.all", side_effect=Exception("Test DB Error")):
            result = await twitch_manager.get_all_streamers()
    ...
    async def test_update_streamer_status_online(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.filter") as mock_filter:
            ...
            mock_update.assert_called_once_with(is_live=True)
    ...
    async def test_update_notification_time_with_stream_id(self, twitch_manager):
        ...
        with patch("utils.twitch_data_manager.TwitchStreamer.filter") as mock_filter, \
            ...
            mock_update.assert_called_once_with(last_notification_time=1234567890, last_stream_id="stream123")
