...
async def test_stats_view_on_timeout() -> None:
    ...
    # max_pages > 1 — чтобы пагинатор был активен
    view = StatsView("Test Title", [("Game1", 3600), ("Game2", 7200)], items_per_page=1)
    view.message = MagicMock()
    view.message.edit = AsyncMock()

    await view.on_timeout()

    # Все кнопки пагинатора должны быть отключены
    assert all(btn.disabled for btn in view._pager.children)
    view.message.edit.assert_called_once_with(view=view)


@pytest.mark.asyncio
async def test_stats_view_previous_button(mock_interaction: MagicMock) -> None:
    """Кнопка 'Назад' листает на предыдущую страницу через change_page."""
    from utils.activity.views import StatsView

    view = StatsView("Test Title", [("Game1", 3600), ("Game2", 7200)], items_per_page=1)
    view.current_page = 1  # Начинаем со второй страницы

    await view.change_page(mock_interaction, -1)

    assert view.current_page == 0
    # CV2: редактируем только view, без embed
    mock_interaction.response.edit_message.assert_called_once_with(view=view)


@pytest.mark.asyncio
async def test_stats_view_next_button(mock_interaction: MagicMock) -> None:
    """Кнопка 'Вперед' листает на следующую страницу через change_page."""
    from utils.activity.views import StatsView
...
async def test_stats_view_previous_button_on_first_page(mock_interaction: MagicMock) -> None:
    ...
    # current_page не изменился, был вызван defer
    assert view.current_page == 0
...
async def test_stats_view_on_timeout_without_message() -> None:
    ...
    # Не должно падать без сообщения; кнопки пагинатора отключаются
    await view.on_timeout()
...
@pytest.mark.asyncio
async def test_stats_view_update_buttons_disables_on_single_page() -> None:
    """На одной странице кнопки пагинатора отключены (prev и next)."""
    from utils.activity.views import StatsView

    view = StatsView("Test Title", [("Game1", 3600)])
    view._update_buttons()

    # Одна страница → обе кнопки задизейблены
    assert all(btn.disabled for btn in view._pager.children)
