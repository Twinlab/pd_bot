...
from utils.music.config import COLORS, FFMPEG_OPTIONS
...
@pytest.mark.asyncio
async def test_music_player_initialization(music_player, mock_bot):
...
@pytest.mark.asyncio
async def test_connect_success(music_player, mock_voice_client):
...
@pytest.mark.asyncio
async def test_connect_already_connected_same_channel(music_player, mock_voice_client):
...
@pytest.mark.asyncio
async def test_connect_move_to_different_channel(music_player, mock_voice_client):
...
@pytest.mark.asyncio
async def test_disconnect(music_player, mock_voice_client, mock_interaction):
...
@pytest.mark.asyncio
async def test_disconnect_without_interaction(music_player, mock_voice_client, mock_text_channel):
...
@pytest.mark.asyncio
async def test_queue_track(music_player, mock_track_info, mock_member, mock_interaction):
    ...
    with patch('utils.music.player.download_track', new_callable=AsyncMock) as mock_download:
        ...
        with patch('pathlib.Path.exists', return_value=True):
            ...
            # edit_original_response может быть вызван несколько раз (например, "Добавляем..." и "Трек добавлен!")
            assert mock_interaction.edit_original_response.await_count >= 1

@pytest.mark.asyncio
async def test_queue_track_download_error(music_player, mock_member, mock_interaction):
    ...
    with patch('utils.music.player.download_track', new_callable=AsyncMock) as mock_download:
        ...
        found = False
        print("edit_original_response.call_args_list:", mock_interaction.edit_original_response.call_args_list)
        ...
        has_error_embed = any(
            "Ошибка" in getattr(call.kwargs.get("embed"), "title", "")
            for call in mock_interaction.edit_original_response.call_args_list
...
@pytest.mark.asyncio
async def test_start_playback_loop(music_player):
...
@pytest.mark.asyncio
async def test_play_next(music_player, mock_voice_client, mock_track):
...
@pytest.mark.asyncio
async def test_pause(music_player, mock_voice_client, mock_interaction):
...
@pytest.mark.asyncio
async def test_resume(music_player, mock_voice_client, mock_interaction):
...
@pytest.mark.asyncio
async def test_skip(music_player, mock_voice_client, mock_interaction, mock_track):
...
@pytest.mark.asyncio
async def test_stop(music_player, mock_interaction):
...
@pytest.mark.asyncio
async def test_show_queue_empty(music_player, mock_interaction):
...
@pytest.mark.asyncio
async def test_show_queue_with_tracks(music_player, mock_interaction, mock_track):
    ...
    # Проверяем вызовы
    mock_interaction.response.send_message.assert_awaited_once()
    assert "Очередь воспроизведения" in mock_interaction.response.send_message.call_args.kwargs['embed'].title
    assert "Сейчас играет" in mock_interaction.response.send_message.call_args.kwargs['embed'].description
    assert "В очереди" in mock_interaction.response.send_message.call_args.kwargs['embed'].description

@pytest.mark.asyncio
async def test_create_now_playing_embed_no_track(music_player):
...
@pytest.mark.asyncio
async def test_create_now_playing_embed_with_track(music_player, mock_track):
...
@pytest.mark.asyncio
async def test_update_now_playing_message_no_channel(music_player):
...
@pytest.mark.asyncio
async def test_update_now_playing_message_new_message(music_player, mock_text_channel):
...
@pytest.mark.asyncio
async def test_send_error_message(music_player, mock_text_channel):
...
@pytest.mark.asyncio
async def test_cleanup(music_player, mock_track):
...
@pytest.mark.asyncio
async def test_cleanup_without_clearing_queue(music_player, mock_track):
