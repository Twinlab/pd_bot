...
from utils.music.ui import PlayerControlView, SearchView, SearchResultSelect
...
def test_update_buttons_playing(player_control_view, mock_music_player):
    ...
    pause_resume_button = discord.utils.get(player_control_view.children, custom_id="music:pause_resume")
...
@pytest.mark.asyncio
async def test_check_voice_channel_success(player_control_view, mock_interaction, mock_music_player):
...
@pytest.mark.asyncio
async def test_check_voice_channel_different_channel(player_control_view, mock_interaction, mock_music_player):
    ...
    assert result is False
    mock_interaction.response.send_message.assert_awaited_once()
    assert "Вы должны быть в том же голосовом канале" in mock_interaction.response.send_message.call_args.args[0]
...
def test_search_result_select_initialization(search_result_select, mock_music_player, mock_interaction, search_entries):
...
@pytest.mark.asyncio
async def test_search_result_select_callback(search_result_select, mock_interaction, mock_music_player, search_entries):
...
def test_search_view_initialization(search_view, mock_music_player, mock_interaction, search_entries):
...
async def test_search_view_interaction_check_different_user(search_entries, mock_music_player):
    ...
    assert "Только пользователь, запустивший поиск" in test_interaction.response.send_message.call_args.args[0]
