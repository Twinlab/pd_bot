...
async def get_match_data(user_links: Dict[str, List[int]], user_id: str, stratz_api_key: str) -> Tuple[Optional[Dict], Optional[Dict], Optional[int], Optional[Dict]]:
    """
    ...
    Returns:
        Кортеж (match_data, weekly_data, match_id, items_dict) или (None, None, None, None) в случае ошибки.
    """
    ...
    logger.debug(f"Поиск последнего матча для Discord ID {user_id} среди Steam ID: {user_links[user_id]}")
    ...
    logger.info(f"Последний матч для {user_id}: ID {latest_match['id']} (Steam ID: {latest_player_id})")
...
async def handle_lastmatch(ctx: commands.Context, user_links_list: List[int], member: Optional[discord.Member] = None):
    ...
    kda_value = (kills + assists) / max(deaths, 1) # Делим на max(deaths, 1) чтобы избежать деления на ноль
    ...
    hero_name = player_data.get('hero', {}).get('shortName', 'unknown_hero') # Используем shortName для URL картинки
    ...
    if items_dict: # Проверяем, что словарь предметов загружен
        ...
        if neutral_id and neutral_id > 0 and neutral_id in items_dict:
            neutral_str = f" | **{items_dict[neutral_id].get('displayName', f'Item {neutral_id}')}**" # Выделяем нейтралку
