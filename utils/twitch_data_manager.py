...
logger = logging.getLogger("bot.twitch_db")
...
class TwitchDataManager:
    ...
    async def add_streamer(self, guild_id: int, channel_id: int, twitch_username: str, twitch_id: str = None) -> bool:
        """
        ...
        Returns:
            bool: True если добавлено, False если уже существует или произошла ошибка
        """
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                async with db.execute(
                    ...
                    if await cursor.fetchone():
                        ...
                        await db.execute(
                            "UPDATE twitch_streamers SET channel_id = ? WHERE guild_id = ? AND twitch_username = ?",
                            (channel_id, guild_id, twitch_username)
                        )
                        await db.commit()
                        logger.info(f"Обновлен канал для стримера {twitch_username} на сервере {guild_id}")
                        return True
                ...
                await db.execute(
                    ...
                    (guild_id, channel_id, twitch_username, twitch_id, is_live, last_notification_time) 
                    VALUES (?, ?, ?, ?, 0, 0)
                    """,
                    (guild_id, channel_id, twitch_username, twitch_id)
                )
                await db.commit()
                logger.info(f"Добавлен стример {twitch_username} для сервера {guild_id}")
                return True
        except Exception as e:
    ...
    async def remove_streamer(self, guild_id: int, twitch_username: str) -> bool:
        """
        Удаляет стримера из отслеживаемых.
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                if cursor.rowcount > 0:
                    logger.info(f"Удален стример {twitch_username} для сервера {guild_id}")
                    return True
                else:
                    logger.warning(f"Стример {twitch_username} не найден для сервера {guild_id}")
                    return False
        except Exception as e:
    ...
    async def get_streamers(self, guild_id: int) -> List[Dict]:
        """
        Получает список всех отслеживаемых стримеров для сервера.
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                async with db.execute(
                    """
                    SELECT guild_id, channel_id, twitch_username, twitch_id, is_live, last_stream_id, last_notification_time
            ...
            logger.debug(f"Получено {len(streamers)} стримеров для сервера {guild_id}")
            return streamers
        except Exception as e:
            logger.error(f"Ошибка при получении стримеров для сервера {guild_id}: {e}", exc_info=True)
            return []
    ...
    async def update_streamer_status(self, twitch_username: str, is_live: bool, stream_id: str = None) -> bool:
        """
        ...
        Args:
            ...
            stream_id: ID текущего стрима (если стример онлайн)
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                logger.debug(f"Обновлен статус стримера {twitch_username}: {'онлайн' if is_live else 'оффлайн'}")
                return True
        except Exception as e:
    ...
    async def update_twitch_id(self, twitch_username: str, twitch_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Ошибка при обновлении Twitch ID для {twitch_username}: {e}", exc_info=True)
            return False
