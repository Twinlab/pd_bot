...
Проект использует **Ruff** как единый инструмент для линтинга и форматирования кода. Ruff заменяет собой black, isort и flake8, обеспечивая высокую скорость работы и совместимость с их правилами.
...
Используйте `ruff` для автоматической сортировки импортов. Настройки находятся в `pyproject.toml`.
...
- Не показывайте детали неожиданных исключений пользователю. Единый обработчик
  возвращает код инцидента; тот же код должен быть в контексте traceback.
- Не помещайте токены, пароли, API-ключи и заголовки авторизации в лог. Общий
  formatter выполняет дополнительную очистку, но это страховка, а не разрешение
  логировать секреты намеренно.
...
- **Личное/служебное** (формы привязок, ошибки, подтверждения, профиль через ПКМ,
  очередь по кнопке) → `ephemeral=True`.
- **Общий контент** (`/profile` с привязанными аккаунтами, now-playing, карточки матчей по отдельным командам, посты, уведомления) →
  публично.
