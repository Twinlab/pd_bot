...
Команду `/tyan` регистрирует `FunCog`. Модуль `utils/tyan/` содержит
генератор, проверку YAML-словарей и карточку Discord; ему не нужна БД.
`utils/tyan_data_manager.py` читает ограниченное настройками окно общей истории
из `tyan_rolls`, выбирает результат и сохраняет его в одной SQLite-транзакции.
Tortoise сериализует транзакции единственного соединения бота, поэтому
параллельные вызовы не обходят дневную уникальность и общий перерыв событий.

Дата выдачи считается по Москве; время записи хранится как UTC без смещения,
как и в остальных таблицах. При чтении менеджер возвращает генератору UTC
с часовым поясом. В базе остаётся готовый текст, поэтому правка словаря
не изменяет уже выданную сегодня карточку.

Словари входят в Docker-образ вместе с `utils/`. Новая таблица требует
[ручной миграции с backup на VM](tyan-migration-plan.md) до публикации образа.
