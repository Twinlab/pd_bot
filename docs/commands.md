...
| Команда | Описание |
|---------|----------|
| `/party <role> <when> <count> <comment>` | Опубликовать сбор пати + разослать DM всем участникам с этой ролью. `when` — целое число **минут** (1..240). |
...
Пример: `/party @гремлины 15 3 идём ранкед`.
...
3. Embed обновляется живьём: первые `count` человек попадают в **«Готовы»**,
   остальные — в **«Начинку»**. Юзер выпадает только когда у него на DM
   не остаётся ни одной реакции; снимёт лишний эмодзи — продолжает быть
   в списке. Первый из начинки занимает освободившееся место.
